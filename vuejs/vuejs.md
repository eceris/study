...
### 뷰 로더
- 웹팩에서 기본으로 제공하는 라이브러리로, 변환 기능을 맡는다.
- 뷰 CLI로 생성된 모든 `.vue`파일에 뷰로더가 적용.
...
#### style scope
- 기본적으로 component는 부모, 자식의 개념이 존재하는데, 자식 컴포넌트에서만 동작하는 style 을 정의하고 싶다면 `scoped`를 지정한다.
...
- API의 기본형식은 `$emit('이벤트 이름')` 이지만, `$emit('이벤트 이름', 인자1, 인자2...)` 의 형식으로 데이터를 전달 가능 
- 다만 value 일뿐, reference가 넘어가진 않음.

#### :key 속성은 v-for 디렉티브를 사용할 때 지정하는 것이 좋다.
- 내부적으로 dom의 순서를 조작하는데 시간을 최소화 하기 위해
