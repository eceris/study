...
- 직접 작성 가능한 낮은 수준의 소켓 프로토콜에서 전송버퍼가 고갈되고 수신버퍼가 꽉찬 애플리케이션에서 TCP 스톨<sup>갑작스런 중단</sup>을 자주 일으키기도 한다.
...
2. **데이터 생산자에 의지하지 말자.**
3. 애플리케이션 수준의 프로토콜에 **제한**을 두자. 
	- 웹서비스, RMI, XML-RPC 등 의 프로토콜은 **엄청난 양의 객체**을 돌려받는데에 취약하다.

# 5. 안정성 패턴

- 안정성 패턴을 더 많이 가졌다고 해서 고장나지 않을 것이라 생각하지마라. 고장은 발생한다. 예상하라.

## 5.1 제한시간을 사용하라.
- 제한시간은 응답이 오지 않을 것이라고 생각될 때, 기다리는 것을 멈추게 하는 메커니즘.
- 잘 배치된 제한시간은 오류를 차단.
	- 항상 poll(), offer(), tryLock()의 제한시간을 갖는 형식을 제공하라.
- 제한 시간은 재시도와 함께 자주 사용되는데, 일시적인 장애가 아니라면 무용지물이다.
- 작업 큐를 올려놓고 나중에 느리게 처리하는 하는 것은 시스템을 훨씬 강하게 한다.

제한 시간 + 차단기 = 시너지가 발생

### 기억할 것
1. 통합지점, 블록된 스레드, 느린응답에 제한시간 패턴을 적용하자.
2. 예측하지 못한 고장으로부터 회복을 위해 제한시간 패턴을 적용하자.
3. 천천히 재시도 하는 것을 고려하라.
	- 작업을 큐에 올려놓은뒤 나중에 재시도하는 방법.
