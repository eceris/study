...
- [1장 HTTP 개관](https://github.com/eceris/study/tree/master/book/http-the-definitive-guide#1-http-%EA%B0%9C%EA%B4%80)
- [2장 URL과 리소스](https://github.com/eceris/study/tree/master/book/http-the-definitive-guide#2-url%EA%B3%BC-%EB%A6%AC%EC%86%8C%EC%8A%A4)
- [3장 HTTP 메시지](https://github.com/eceris/study/tree/master/book/http-the-definitive-guide#3-http-%EB%A9%94%EC%8B%9C%EC%A7%80)
- [4장 커넥션 관리](https://github.com/eceris/study/tree/master/book/http-the-definitive-guide#4-%EC%BB%A4%EB%84%A5%EC%85%98-%EA%B4%80%EB%A6%AC)
...
#### 조건부 요청 헤더
- 예를 들어 클라이언트가 이미 문서의 사본을 갖고 있다면, 다를 경우에만 전송해달라고 조건부로 요청할 수 있다.
...
#### 프락시 요청 헤더
- 프락시가 점점 흔헤지면서 그들의 기능을 돕기 위한 헤더.
...
#### 협상 헤더
- 서버가 독일어와 영어로 번역된 html 문서를 제공한다면, 서버와 클라이언트가 어떤 표현을 택할 것 인지에 대한 협상을 할 수 있도록 지원.
...
#### 컨텐츠 헤더
- 엔터티 컨텐츠에 대한 구체적인 정보를 제공. 웹 브라우저는 내용 유형을 기술한 Content-Type 헤더를 보고 그 객체를 어떻게 보여줄지 결정할 수 있음.
...
#### 엔터티 캐싱 헤더
- 리소스에 대해 캐시된 사본이 아직 유효한 지에 대한 정보와 캐시된 리소스가 더이상 유효하지 않게 되는 시점을 잘 추정하기 위한 단서를 제공.
...
## TCP 커넥션 
- TCP는 세그먼트라는 단위로 데이터 스트림을 잘게 나누고, 세그먼트를 IP패킷이라고 불리는 봉투에 담아 데이터를 전송.

### 소켓 프로그래밍
- 소켓 API를 통해 기본적인 네트워크 프로토콜의 핸드쉐이킹, 그리고 TCP 데이터 스트림과 IP패킷 간에 분할 및 재조립에 대한 모든 세부사항을 외부에 숨김.

## TCP의 성능에 대한 고려
- 성능에 대한 중요한 요소는 아래와 같음
```
- TCP 커넥션의 핸드셰이크 설정
...
- Connection 헤더에 있는 모든 헤더필드는 메시지를 다른 곳에 전달하는 시점에 `삭제` 되어야 함. 홉<sup>hop-by-hop</sup>별 헤더이므로
- Proxy-Authenticate, Proxy-Connection, Transfer-Encoding, Upgrade 도 마찬가지.

## 병렬 커넥션 
- 여러 개의 커넥션을 맺음으로써 여러개의 HTTP 트랜잭션을 병렬로 처리.
- 대역폭으로 인해 성능이 떨어질수 있고, 브라우져에서 최대 6 ~ 8개의 병렬 커넥션만 허용.
...
## 커넥션 끊기에 대한 미스터리
- 클라이언트에서 이미 끊긴 입력 채널에 데이터를 전송하면 운영체제는 `TCP 'connection reset by peer'` 라는 메시지를 클라이언트에 보냄.
- 보통 우아한 커넥션 끊기는 애플리케이션이 자신의 출력 채널을 먼저 끊고 다른 쪽에 있는 기기의 출력 채널이 끊기는 것을 기다리는 것.
