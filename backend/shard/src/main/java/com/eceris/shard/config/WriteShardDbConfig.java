...
//)
public class WriteShardDbConfig {

    private final WriteShardDbProperties dbproperties;

    private HikariDataSource toHikariDataSource(Integer index) {

        HikariDataSource hikariDataSource = new HikariDataSource();
        WriteShardDbProperties.Shard shard = dbproperties.getShards().get(index);
...
//
//    @Bean
//    public PlatformTransactionManager shardTransactionManager(DataSource writeShardDataSource) {
//        JpaTransactionManager jpaTransactionManager = new JpaTransactionManager();
//        jpaTransactionManager.setEntityManagerFactory(shardEntityManager(writeShardDataSource).getObject());
//        return jpaTransactionManager;
//    }
//
//    @Bean
//    LocalContainerEntityManagerFactoryBean shardEntityManager(DataSource writeShardDataSource) {
//
...
//        localContainerEntityManagerFactoryBean.setDataSource(writeShardDataSource);
...
//    }
    ...
    @Bean
    public DataSource writeShardDataSource() throws SQLException {
    ...
    private ShardingRuleConfiguration getDBShardingRuleConfiguration() {
        final PreciseShardingAlgorithm<String> shardingAlgorithm = (availableTargetNames, shardingValue) -> {
            final Integer shardIndex = Math.abs((shardingValue.getValue().hashCode() % dbproperties.getShards().size()));
